...
<div>
  ...
  foreach ($goods_total as $result){
    ...
    $qty = $result['ave']-$result['buffer_qty'];
   	...
   	echo '<td style="text-align:center;"><a href="?action=send&sku_order='.htmlspecialchars($result['sku']) .'&id='.$result['sku_id'].'&qty=' .$result['pack_qty'].'"class="btn btn-default btn-primary">Order</a></td>
    </tr>
    </form>';
