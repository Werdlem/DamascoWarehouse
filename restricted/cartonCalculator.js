...
},
{
  ...
  panelW: 1
      ...
      var res = (($scope.width * $scope.styleSelect.panelW) + ($scope.height *1)+10); 
      if(isNaN(res)){
        return null;
      } 
     ...
     $scope.calcBoardLength = function(){
      var res = (($scope.width * $scope.configSelect.panelW) + ($scope.length * $scope.configSelect.panelL) + 40+9);
      if(isNaN(res)){
        return null;
      } 
